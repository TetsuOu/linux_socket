...
class HandleThread {
  public:
    HandleThread() 
    : epoll_fd_(epoll_create1(EPOLL_CLOEXEC)),//1、创建epoll实例
      epoll_events_(EVENTSMAXNUM),
      thread_(std::bind(&HandleThread::work, this)) {
      assert(epoll_fd_ > 0);
      thread_.detach();
    }
    //这里的bind函数不是socket里的bind函数，而是functional里的bind函数，返回一个函数对象
    //这里std::bind绑定了类成员函数，第一个参数是成员函数指针，第二个参数是对象地址
    ~HandleThread() {
      close(epoll_fd_);
    }
    // 线程实际运行函数
...
void HandleThread::work() {
  for(;;) {
    ...
    //3、等待事件发生，没有事件时，调用者进程会被挂起，等到事件发生/超时后返回
    if (event_count < 0) {
...
void HandleThread::addFd(int fd) {
  ...
  //ctl  2、管理监听的描述符，并注册要监听的事件
  //epoll_fd_ : epoll_create创建的epoll_fd
  //EPOLL_CTL_ADD : 要操作的类型为注册事件
  //fd : 要操作的文件描述符
  //event : 要注册的事件类型
}
